...
\begin{figure}[htbp]
\centering
% resizebox ensures it never overflows the text width
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  font=\small,
  year/.style={
    rectangle, rounded corners=3pt,
    fill=gray!20, draw=gray!50,
    minimum width=1.1cm, minimum height=0.6cm,
    align=center, font=\bfseries\small
  },
  aitem/.style={   % architecture / generation papers — blue
    rectangle, rounded corners=3pt,
    fill=blue!10, draw=blue!40,
    text width=2.4cm, minimum height=0.65cm,
    align=center, font=\footnotesize
  },
  bitem/.style={   % retrieval / evaluation papers — teal
    rectangle, rounded corners=3pt,
    fill=teal!10, draw=teal!40,
    text width=2.4cm, minimum height=0.65cm,
    align=center, font=\footnotesize
  },
  tick/.style={gray!50, line width=0.5pt},
  conn/.style={line width=0.5pt}
]

% ---- axis (15 cm total, years evenly spaced at 2.14 cm) ----
\draw[-{Stealth[length=6pt]}, line width=1.2pt, gray!60]
  (-0.3, 0) -- (15.2, 0);

\foreach \x/\yr in {0/2020, 2.14/2021, 4.28/2022, 6.42/2023, 8.56/2024, 10.70/2025, 13.50/2026}{
  \draw[tick] (\x,-0.3) -- (\x,0.3);
  \node[year] at (\x, 0) {\yr};
}

% ---- ABOVE axis: architecture / generation papers ----
% 2020: RAG (lower) + DPR (upper)
\node[aitem] (rag)      at (0,    1.4)  {RAG\\{\tiny Lewis et al.}};
\node[aitem] (dpr)      at (0,    2.45) {DPR\\{\tiny Karpukhin et al.}};
% 2022: HyDE
\node[aitem] (hyde)     at (4.28, 1.4)  {HyDE\\{\tiny Gao et al.}};
% 2023: Self-RAG
\node[aitem] (selfrag)  at (6.42, 1.4)  {Self-RAG\\{\tiny Asai et al.}};
% 2024: GraphRAG (lower) + RAPTOR (upper)
\node[aitem] (graphrag) at (8.56, 1.4)  {GraphRAG\\{\tiny Edge et al.}};
\node[aitem] (raptor)   at (8.56, 2.45) {RAPTOR\\{\tiny Sarthi et al.}};
% 2025: Chain-of-Retrieval
\node[aitem] (chain)    at (10.70,1.4)  {Chain-of-Retr.\\{\tiny Wang et al.}};
% 2026: A-RAG
\node[aitem] (arag)     at (13.50,1.4)  {A-RAG\\{\tiny Du et al.}};

% ---- BELOW axis: retrieval models + eval frameworks ----
\node[bitem] (colbert)   at (0,    -1.4) {ColBERT\\{\tiny Khattab \& Zaharia}};
\node[bitem] (beir)      at (2.14, -1.4) {BEIR\\{\tiny Thakur et al.}};
\node[bitem] (colbertv2) at (4.28, -1.4) {ColBERTv2\\{\tiny Santhanam et al.}};
\node[bitem] (ragas)     at (6.42, -1.4) {RAGAS\\{\tiny Es et al.}};
\node[bitem] (crag)      at (8.56, -1.4) {Corr.\ RAG\\{\tiny Yan et al.}};
\node[bitem] (agentic)   at (10.70,-1.4) {Agentic RAG\\{\tiny Singh et al.}};
\node[bitem] (rarerag)   at (13.50,-1.4) {\textbf{RARE-RAG}\\{\tiny this work}};

% ---- connectors: tick → node ----
\foreach \n/\x in {rag/0, dpr/0, hyde/4.28, selfrag/6.42, graphrag/8.56, raptor/8.56, chain/10.70, arag/13.50}
  \draw[conn, blue!35]  (\x, 0.3) -- (\n.south);
\foreach \n/\x in {colbert/0, beir/2.14, colbertv2/4.28, ragas/6.42, crag/8.56, agentic/10.70, rarerag/13.50}
  \draw[conn, teal!45] (\x,-0.3) -- (\n.north);
...
\begin{tikzpicture}[
  font=\small,
  % ---- node styles ----
  start/.style ={ellipse, draw=blue!55, fill=blue!8,  thick,
                 minimum width=2.6cm, minimum height=0.85cm, align=center},
  proc/.style  ={rectangle, rounded corners=4pt,
                 draw=blue!50, fill=blue!8, thick,
                 minimum width=3.0cm, minimum height=0.9cm, align=center},
  dec/.style   ={diamond, draw=orange!65, fill=orange!8, thick,
                 aspect=3.0, minimum width=3.6cm, minimum height=0.9cm,
                 align=center, inner sep=0pt},
  outbox/.style={rectangle, rounded corners=4pt,
                 draw=green!55, fill=green!10, thick,
                 minimum width=3.0cm, minimum height=0.9cm, align=center},
  side/.style  ={rectangle, rounded corners=3pt,
                 draw=red!45, fill=red!6, thick,
...
% ================================================================
%  NODE PLACEMENT
%  Left column  x = 0  : main happy-path (top → bottom)
%  Right column x = 6  : retrieval sub-flow
%  Far right    x = 10 : discard exit
%  Left loop    x = -3 : "no" bypass
%  Far-left loop x= -4.5 : "not useful" feedback
% ================================================================

% -- Left column --
\node[start] (q)     at ( 0,    0  ) {Query $q$};
\node[dec]   (retd)  at ( 0,  -2.2 ) {\texttt{[Retrieve?]}};
\node[proc]  (gen)   at ( 0,  -6.5 ) {Generate\\segment};
\node[dec]   (issup) at ( 0,  -8.7 ) {\texttt{[IsSup?]}};
\node[dec]   (isuse) at ( 0, -10.9 ) {\texttt{[IsUse?]}};
\node[outbox] (ans)   at ( 0, -13.0 ) {Final answer $a$};

% -- Right column --
\node[proc]  (retr)  at ( 6,  -2.2 ) {Retrieve\\passages};
\node[dec]   (isrel) at ( 6,  -4.5 ) {\texttt{[IsRel?]}};
\node[side]  (disc)  at (10,  -4.5 ) {Discard\\passage};
\node[side]  (regen) at ( 6,  -8.7 ) {Regenerate\\segment};

% ================================================================
%  ARROWS  — each path is routed to avoid any crossing
% ================================================================

% 1. q → retd  (straight down, x=0)
\draw[arr] (q) -- (retd);

% 2. retd --yes→ retr  (straight right at y=-2.2)
\draw[arr] (retd.east) -- node[lbl, above=2pt]{yes} (retr.west);

% 3. retd --no→ gen.west  (LEFT bypass: go left to x=-3, down, then right into gen.west)
%    Path stays entirely left of x=0, never crosses the right-column paths.
\draw[arr] (retd.west)
    -- ++(-1.8, 0)          % horizontal left  → (-1.8, -2.2)
    -- ++(0,   -4.3)        % vertical down    → (-1.8, -6.5)
    -- (gen.west)           % horizontal right → gen.west
    node[lbl, pos=0.08, above=2pt]{no};

% 4. retr → isrel  (straight down at x=6)
\draw[arr] (retr) -- (isrel);

% 5. isrel --not rel.→ disc  (straight right at y=-4.5)
\draw[arr] (isrel.east) -- node[lbl, above=2pt]{not rel.} (disc.west);

% 6. isrel --relevant→ gen.north
%    Exit isrel.west → go left to x=0 → descend to gen.north
%    The vertical segment at x=0 (y: -4.5 → -6.05) is clear:
%    q→retd occupies only y=0 to -1.8, and "no" left-bypass is at x=-1.8.
\draw[arr] (isrel.west)
    -- ++(-6, 0)            % horizontal left  → (0, -4.5)
    -- (gen.north)          % vertical down    → gen.north (0, -6.05)
    node[lbl, pos=0.35, above=2pt]{relevant};

% 7. gen → issup  (straight down, x=0)
\draw[arr] (gen) -- (issup);

% 8. issup --not sup.→ regen  (straight right)
\draw[arr] (issup.east) -- node[lbl, above=2pt]{not sup.} (regen.west);

% 9. regen --retry→ gen.east
%    Exit regen.east → far right (x=8.5) → up to y=-6.5 → left into gen.east.
%    Horizontal at y=-6.5 runs x=8.5→1.5, well clear of "relevant" (x=0, y=-4.5→-6.05)
%    and of "no" bypass (x=-1.8).
\draw[arr] (regen.east)
    -- ++(2.5, 0)           % right  → (8.5, -8.7)
    -- ++(0,   2.2)         % up     → (8.5, -6.5)
    -- (gen.east)           % left   → gen.east (1.5, -6.5)
    node[lbl, pos=0.5, above=2pt]{retry};

% 10. issup --supported→ isuse  (straight down, x=0)
\draw[arr] (issup) -- node[lbl, right=3pt]{supported} (isuse);

% 11. isuse --useful→ ans  (straight down, x=0)
\draw[arr] (isuse) -- node[lbl, right=3pt]{useful} (ans);

% 12. isuse --not useful→ retd
%     Exit isuse.west → far left (x=-4.5) → up → right → retd.west.
%     Stays to the LEFT of the "no" bypass (x=-1.8) at all times.
\draw[arr] (isuse.west)
    -- ++(-3.3, 0)          % left  → (-3.3, -10.9)
    -- ++(0,    9.4)        % up    → (-3.3,  -1.5)
    -- ++(2.1,  0)          % right → (-1.2,  -1.5)
    -- (retd.west)          % down  → retd.west (-1.2, -2.2)
    node[lbl, pos=0.07, below=2pt]{not useful};
...
% ---- bubbles with carefully separated labels ----
% Vanilla RAG  — left, label to the right to stay inside plot
\node[dot, fill=blue!20,   draw=blue!60]   (van)   at (1.2, 1.2) {};
\node[font=\footnotesize, right=4pt]  at (van)   {Vanilla RAG};

% HyDE — label above
\node[dot, fill=cyan!20,   draw=cyan!60]   (hyde)  at (3.0, 1.6) {};
\node[font=\footnotesize, above=4pt]  at (hyde)  {HyDE};

% Corrective RAG — label above
\node[dot, fill=cyan!25,   draw=cyan!60]   (crag)  at (5.5, 2.0) {};
\node[font=\footnotesize, above=4pt]  at (crag)  {Corr. RAG};

% Self-RAG — moved to (4.8, 3.5) so Pareto label has room below it
\node[dot, fill=orange!25, draw=orange!60] (self)  at (4.8, 3.5) {};
\node[font=\footnotesize, left=4pt]   at (self)  {Self-RAG};

% Graph RAG — label to the left
\node[dot, fill=purple!20, draw=purple!50] (graph) at (3.2, 4.5) {};
\node[font=\footnotesize, left=4pt]   at (graph) {Graph RAG};

% Iter. Multi-hop — label to the right
\node[dot, fill=teal!20,   draw=teal!60]   (iter)  at (6.2, 5.2) {};
\node[font=\footnotesize, right=4pt]  at (iter)  {Iter. Multi-hop};

% MADAM-RAG — label above
\node[dot, fill=red!15,    draw=red!50]    (madam) at (7.5, 6.8) {};
\node[font=\footnotesize, above=4pt]  at (madam) {MADAM-RAG};

% RARE-RAG — larger bubble, label above
\node[dot, fill=green!25,  draw=green!60,
      minimum size=0.80cm, line width=1.5pt] (rare)  at (9.0, 5.6) {};
\node[font=\footnotesize\bfseries, above=4pt] at (rare) {\rarerag{}};

% Pareto frontier curve — positioned BELOW Self-RAG to avoid overlap
\draw[green!50!black, thick, dashed, rounded corners=6pt]
  (1.2,1.2) .. controls (3.5,2.0) and (6.5,4.2) .. (9.0,5.6);
% Label placed at a clear gap between CRAG and Self-RAG
\node[font=\footnotesize, green!60!black, rotate=24] at (4.8, 2.5)
  {Pareto frontier};
