...
memorisation of facts --- appears to have remained largely unchallenged since,
and underpins the architectures surveyed in the remainder of this chapter.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}llXX@{}}
    ...
    Adaptive RAG
      & 2024
      & Complexity-based routing to per-query pipeline depth
      & Classifier needs labelled complexity data \\
...
\cref{fig:vanilla_rag} illustrates the flow; its seven nodes group into the
three stages of retrieval, augmentation, and generation.
...
In the Rewrite--Retrieve--Read formulation~\cite{ma2023rewrite} the rewriter is a
small language model trained with reinforcement learning against downstream answer
quality; in practice the rewriting step is often delegated to a~prompted
general-purpose LLM instead, which is the variant implemented in this work
(\cref{sec:exp_setup}).
...
Self-RAG~\cite{asai2024selfrag} represents a~qualitative shift from the
retrieve-then-generate paradigm to a~\emph{generate-with-retrieval-on-demand} paradigm.
...
That fine-tuning requirement is the reason the \texttt{self\_reflection} mode
evaluated in this thesis is a~simplified, prompt-based approximation rather than a
faithful reproduction: it scores and re-drafts the whole answer with ordinary LLM
calls instead of emitting per-token reflection tokens from a~fine-tuned decoder
(\cref{sec:query_pipeline,cha:evaluation}).
...
It is also the feature this work does not implement.
The \texttt{corrective\_rag} mode evaluated in \cref{cha:evaluation} replaces the
fine-tuned evaluator with a~threshold on the raw hybrid retrieval score, and
replaces the web-search fallback with a~broadened re-query of the same corpus.
A~closed-corpus advisory system cannot responsibly cite the open web on a
drug-safety question, so the omission is deliberate --- but it means the mode as
evaluated here tests a~relevance-gated re-retrieval strategy rather than CRAG's
central claim.
...
A~more recent systematisation by Mishra \etal~\cite{mishra2026sokagentic} formalises
agentic RAG as a~partially observable sequential decision process and classifies
systems by planning mechanism, retrieval orchestration, memory paradigm and tool
behaviour.
Among the systemic risks they identify in autonomous retrieval loops is the
propagation of hallucinated content between steps --- the failure mode that
motivates the post-generation grounding check proposed in \cref{cha:rare_rag}.
...
\caption[RAG families positioned by retrieval adaptivity and generation complexity.]{Qualitative positioning of the surveyed RAG families along two conceptual axes: retrieval adaptivity (x) and generation complexity (y). Coordinates are illustrative rather than measured, and marker size carries no meaning; RARE-RAG (highlighted) sits at the high-adaptivity end.}
