...
Relevance is computed as the \emph{MaxSim} score: for each query token, find the
maximum cosine similarity across all passage token embeddings, then sum these maxima.
...
Claims are extracted from the answer by an LLM, and each claim is checked against
the context by the same LLM acting as a natural language inference judge.
...
original question.
It is computed by generating several questions from the answer using an LLM and
measuring the average cosine similarity between the generated questions and the
original query.
A high-relevance answer stays on-topic; a low-relevance one drifts into tangential
information.

\textbf{Context Recall} measures how much of the information in the gold answer is
