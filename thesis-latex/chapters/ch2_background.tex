...
\cref{fig:rag_timeline} summarises the key milestones.
...
\cref{tab:rag_families} organises them into eight families, ordered by approximate
...
Its principal advantage is simplicity: a single retrieval pass followed by a single
generation call keeps latency low and the system easy to reason about.
...
latency compared to vanilla RAG.
...
before continuing generation; \texttt{[IsRel]}/\texttt{[IsNotRel]} judges whether a
retrieved passage is relevant to the query; \texttt{[IsSup]}/\texttt{[IsNotSup]} judges
whether the generated statement is supported by the retrieved passage; and
\texttt{[IsUse]} rates the overall utility of a generated segment.
...
\caption[Self-RAG decision flow~\cite{asai2023selfrag}.]{Self-RAG decision flow~\cite{asai2023selfrag}. The model generates reflection tokens (\texttt{[Retrieve?]}, \texttt{[IsRel?]}, \texttt{[IsSup?]}, \texttt{[IsUse?]}) to control retrieval, passage selection, and regeneration.}
\label{fig:selfrag}
\end{figure}
...
The re-retrieval mechanism adds a moderate latency overhead compared to vanilla RAG,
but the quality gain on questions where the initial retrieval fails can be substantial.
In practice, the web-search fallback provides the greatest benefit on queries about
recent events or narrow sub-domains not well represented in a static corpus.
...
generation call, and complex questions may require several hops before a complete
evidence chain is assembled.
Empirically, the iterative process does not always recover evidence that a single
broad retrieval pass would have found, as each sub-query narrows the search scope.
...
competitive latency despite higher total token counts, as the wall-clock cost is
determined by the critical path rather than total compute.
...
This limitation is acknowledged in \cref{sec:limitations}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/arch_comparison.png}
