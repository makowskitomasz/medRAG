...
($9 \times 100$ questions at five difficulty levels), using lexical metrics
(Token~F1, EM, ROUGE-L) and RAGAS-based indicators (faithfulness, answer correctness,
answer relevance), together with latency analysis.

The \textbf{\rarerag{} architecture} --- a new RAG pipeline combining query complexity
routing, hybrid BM25+vector retrieval, set-wise evidence selection, and a grounding
verifier with abstention; \rarerag{} achieves the highest faithfulness (0.976) on
HotpotQA and the highest answer correctness on expert-level DDI questions (L5: 0.470).
